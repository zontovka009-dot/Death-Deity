...
def add_role(name: str):
    cur.execute("INSERT INTO roles (name, is_free) VALUES (?, 1)", (name,))
    conn.commit()
...
def toggle_role(role_id: int):
    cur.execute("SELECT is_free FROM roles WHERE id = ?", (role_id,))
    row = cur.fetchone()
    if row:
        new_val = 0 if row[0] == 1 else 1
        cur.execute("UPDATE roles SET is_free = ? WHERE id = ?", (new_val, role_id))
        conn.commit()


def get_free_roles():
    cur.execute("SELECT * FROM roles WHERE is_free = 1")
    return cur.fetchall()
...
def create_application(user_id, birth_date, role, about):
    cur.execute(
        "INSERT INTO applications (user_id, birth_date, role, about, created_at) VALUES (?, ?, ?, ?, ?)",
        (user_id, birth_date, role, about, now()),
    )
    conn.commit()
