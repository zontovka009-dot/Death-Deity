...
import texts as t
import utils
from states import AdminWriteToUser, AdminEditTemplate
...
async def view_applications_menu(message: Message, state: FSMContext):
    await utils.replace_message(message, state, "Что вас интересует?", kb.admin_applications_menu())
...
async def view_processed_applications(message: Message):
    ...
    if not apps:
        ...
        return
    for app in apps:
        status_label = {"approved": "✅ Одобрена", "rejected": "❌ Отклонена"}.get(app[3], app[3])
...
async def app_accept(call: CallbackQuery):
    ...
    newcomer_text = f"👋 Новый участник!\n\n{utils.esc(app[2])}"
...
async def view_tickets_menu(message: Message, state: FSMContext):
    await utils.replace_message(message, state, "Что вас интересует?", kb.admin_tickets_menu())
...
async def view_admins(message: Message, state: FSMContext):
    ...
    await utils.replace_message(message, state, text, kb.back_menu())
...
async def edit_template_start(message: Message, state: FSMContext):
    ...
    await utils.replace_message(message, state, "Пришлите новый текст анкеты:", kb.back_menu())
...
async def edit_photo_start(message: Message, state: FSMContext):
    ...
    await utils.replace_message(message, state, "Пришлите фото для анкеты:", kb.back_menu())
